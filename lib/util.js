 ...
 * defined dependencies. That is, only string literals. This applies to both
 * the standard AMD-style `require` with dependency list and callback function,
 * as well as the CommonJS Wrapper form of `require` in which a single string
 * literal is provided.
 *
 * @public
 * @param  {ASTNode} node - CallExpression node to test
...
function isStaticRequire(node) {
    ...
    return args.length === 1 && isStringLiteral(args[0]) ||
           args.length === 2 && isArrayExpr(args[0]) && isStringLiteralArray(args[0]) && isFunctionExpr(args[1]);
}
